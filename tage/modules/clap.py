...
class CLAP_base(torch.nn.Module):
    def __init__(self, checkpoint_path=f'{home_dir}/.cache/tage/ckpt', requires_grad=False, model_arch='HTSAT-base',
        ...
        ckpt_path = os.path.join(checkpoint_path, chkpt)
        if not os.path.exists(ckpt_path):
            print("Download pretrained checkpoints of the CLAP model.")
            os.makedirs(f"{home_dir}/.cache/tage/ckpt", exist_ok=True)
            os.system(f"wget -P {checkpoint_path} https://huggingface.co/lukewys/laion_clap/resolve/main/music_speech_audioset_epoch_15_esc_89.98.pt")
        from clap_module.factory import load_state_dict  # type: ignore
        pkg = load_state_dict(ckpt_path)
    ...
    def _forward_features(self, x, longer_idx=None):
        r"""A deprecated optimization for using a hierarchical output from different blocks."""
    ...
    def _convert_audio(wav: torch.Tensor, from_rate: float,
        ...
        r"""Convert audio to new sample rate and number of audio channels."""
    ...
    def _convert_audio_channels(wav: torch.Tensor, channels: int = 2) -> torch.Tensor:
        r"""Convert audio to the given number of channels.
