...
from tage.modules.panns.pytorch_utils import (
    do_mixup,
    interpolate,
...
class Cnn14(nn.Module):
    def __init__(
        ...
        if not os.path.exists("%s/.cache/tage/ckpt/Cnn14_mAP=0.431.pth" % (home_dir)):
            print("Download pretrained checkpoints of Cnn14.")
            os.makedirs("%s/.cache/tage/ckpt" % (home_dir), exist_ok=True)
            os.system("wget -P %s/.cache/tage/ckpt/ %s" % (home_dir,"https://zenodo.org/record/3576403/files/Cnn14_mAP%3D0.431.pth"))
            os.system("wget -P %s/.cache/tage/ckpt/ %s" % (home_dir,"https://zenodo.org/record/3987831/files/Cnn14_16k_mAP%3D0.438.pth"))

        # self.init_weight()
        if sample_rate == 16000:
            state_dict = torch.load("%s/.cache/tage/ckpt/Cnn14_16k_mAP=0.438.pth" % home_dir)
            self.load_state_dict(state_dict["model"])
        elif sample_rate == 32000:
            state_dict = torch.load("%s/.cache/tage/ckpt/Cnn14_mAP=0.431.pth" % home_dir)
            self.load_state_dict(state_dict["model"])
...
class Cnn14_16k(nn.Module):
    def __init__(
        ...
        # self.init_weight()
        home_dir = os.path.expanduser("~")
        if not os.path.exist("%s/.cache/tage/ckpt/Cnn14_mAP=0.431.pth" % home_dir):
            print("Download pretrained checkpoints of Cnn14.")
